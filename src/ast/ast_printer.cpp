...
namespace flux::ast {
    ...
    void ASTPrinter::print_statement(const Stmt &stmt) {
        if (auto let = dynamic_cast<const LetStmt *>(&stmt)) {
            indent();
            if (let->is_const) std::cout << "Const ";
            else {
                std::cout << "Let ";
                if (let->is_mutable) std::cout << "mut ";
            }
        ...
        if (auto fl = dynamic_cast<const ForStmt*>(&stmt)) {
            ...
            if (!fl->var_type.empty()) std::cout << " : " << fl->var_type;
            std::cout << " in\n";
            indent_level_++;
    ...
    void ASTPrinter::print_expression(const Expr &expr) {
        ...
        if (auto rng = dynamic_cast<const RangeExpr *>(&expr)) {
            ...
            std::cout << "Range\n";
            indent_level_++;
            if (rng->start) print_expression(*rng->start);
            if (rng->end) print_expression(*rng->end);
            indent_level_--;
            return;
        }
