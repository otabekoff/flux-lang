...
namespace flux {
    ...
    ast::ExprPtr Parser::parse_primary() {
        ...
        if (tok.kind == TokenKind::Minus || tok.kind == TokenKind::Bang ||
            tok.kind == TokenKind::Amp || tok.kind == TokenKind::Tilde) {
        ...
        while (true) {
            if (match(TokenKind::ColonColon)) {
                const std::string member = expect(TokenKind::Identifier, "expected member name after '::'").lexeme;
                ...
                advance(); // consume '['
                ast::ExprPtr index = parse_expression();
                expect(TokenKind::RBracket, "expected ']' after index");
                expr = std::make_unique<ast::BinaryExpr>(TokenKind::LBracket, std::move(expr), std::move(index));
            } else {
                break;
            }
        }
    ...
    ast::Module Parser::parse_module() {
        ...
        while (!is_at_end()) {
            ...
            throw DiagnosticError("expected top-level declaration (func, struct, class, enum, impl, trait, type)", peek().line, peek().column);
        }
    ...
    ast::StmtPtr Parser::parse_statement() {
        ...
        if (peek().kind == TokenKind::Identifier) {
            ...
            if (assign_op == TokenKind::Assign ||
                assign_op == TokenKind::PlusAssign ||
                assign_op == TokenKind::MinusAssign ||
                assign_op == TokenKind::StarAssign ||
                assign_op == TokenKind::SlashAssign ||
                assign_op == TokenKind::PercentAssign ||
                assign_op == TokenKind::AmpAssign ||
                assign_op == TokenKind::PipeAssign ||
        ...
        if (peek().kind == TokenKind::Assign ||
            peek().kind == TokenKind::PlusAssign ||
            peek().kind == TokenKind::MinusAssign ||
            peek().kind == TokenKind::StarAssign ||
            peek().kind == TokenKind::SlashAssign ||
            peek().kind == TokenKind::PercentAssign) {
            TokenKind op = peek().kind;
            advance();
            ast::ExprPtr value = parse_expression();
            expect(TokenKind::Semicolon, "expected ';' after assignment");
    ...
    ast::StmtPtr Parser::parse_let_statement() {
        ...
        const std::string type_name = parse_type();
    ...
    ast::StmtPtr Parser::parse_for_statement() {
        ...
        return std::make_unique<ast::ForStmt>(std::move(var_name), std::move(var_type), std::move(iterable), std::move(body));
    }
    ...
    ast::EnumDecl Parser::parse_enum_declaration(bool is_public) {
        ...
        while (!match(TokenKind::RBrace)) {
            const std::string variant_name = expect(TokenKind::Identifier, "expected variant name").lexeme;
    ...
    ast::ImplBlock Parser::parse_impl_block() {
        ...
        while (!match(TokenKind::RBrace)) {
            bool is_pub = false;
            if (check_visibility()) {
                if (peek().lexeme == "pub" || peek().lexeme == "public") is_pub = true;
                advance();
            }
    ...
    std::vector<std::string> Parser::parse_type_params() {
        ...
        if (match(TokenKind::Less)) {
            do {
                std::string param = expect(TokenKind::Identifier, "expected type parameter name").lexeme;
    ...
    std::string Parser::parse_where_clause() {
        ...
        while (peek().kind != TokenKind::LBrace && !is_at_end()) {
            clause += peek().lexeme;
            advance();
            if (peek().kind != TokenKind::LBrace) clause += " ";
        }
