    ...
    std::vector<Token> Lexer::tokenize() {
        ...
        while (!is_at_end()) {
            ...
            if (std::isdigit(static_cast<unsigned char>(c))) {
                ...
                if (c == '0') {
                    ...
                    if (next == 'x' || next == 'X') {
                        ...
                        while (!is_at_end() && (std::isxdigit(static_cast<unsigned char>(peek())) || peek() == '_')) {
                            char n = advance();
                            if (n != '_') number.push_back(n);
                        }
                        ...
                        while (!is_at_end() && (std::isdigit(static_cast<unsigned char>(peek())) || peek() == '_')) {
                            char n = advance();
                            if (n != '_') number.push_back(n);
                        }
                    }
            ...
            throw DiagnosticError(
                std::string("unexpected character '") + bad + "'",
                err_line,
                err_col
            );
        }
